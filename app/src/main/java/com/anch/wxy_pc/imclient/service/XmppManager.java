...
public class XmppManager {
     ...
     * 连接openfire开始登陆
     */
    public void connectOpenfireSer(Context context, String userName, String userPass, XmppCallBack xmppCallBack) {

        if (isAlreadyLogin()) {
            Toast.makeText(context, "已经登陆", Toast.LENGTH_SHORT).show();
            return;
        }
        if (isOnline(userName)) {
            Toast.makeText(context, "已经在线", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        protected void onPostExecute(Integer result) {
            switch (result) {
                ...
                case XmppConstant.XMPP_LOGIN_FAILD://登陆失败
                    xmppCallBack.onFail(XmppConstant.XMPP_LOGIN_FAILD);
                    break;
                case XmppConstant.XMPP_CON_SER_TIMEOUT://连接失败
                    xmppCallBack.onFail(XmppConstant.XMPP_CON_SER_TIMEOUT);
                    break;
            }
        }
    }
...
//        }
        if (chat != null) {
            try {
                ...
                e.printStackTrace();
                Toast.makeText(context, "服务器响应失败，导致发送失败", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void applyAddFriend(String from) {
        ...
        } else sendUpdateOtherContactsBrocast(from);
    }
    ...
    private class MConnectionListener implements ConnectionListener {
        ...
        public void connectionClosed() {
            xmppCallBack.onFail(XmppConstant.XMPP_CON_SER_EXIT);
        }
        ...
        public void entriesAdded(Collection<String> collection) {
            ...
            for (String jid : collection) {
                ...
                if (XmppConstant.XMPP_LAUNCH_ADD_FRIEND1.equals("XMPP_LAUNCH_ADD_FRIEND1")) {
                    ...
                    sendUpdateOwnContactsBrocast();
                } else sendUpdateOtherContactsBrocast(jid);
                break;
            }
        }
    ...
    //发送删除联系人之后更新其他人联系人广播
    private void sendUpdateOtherContactsBrocast(String jid) {
