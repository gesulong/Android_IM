...
//            currentUser = intent.getStringExtra(Constanst.SKIP_ADD_FRIENT_PASS_USER);
    ...
    public void onClick(View v) {
        if (addFriendEt.length() == 0)
            Toast.makeText(AddFriendAct.this, "请输入好友账户", Toast.LENGTH_SHORT).show();
        else {
            addFriend();
        }
    }
            ...
            protected void onPostExecute(Integer result) {
                ...
                switch (result) {
                    case XmppConstant.XMPP_ADD_FRIEND_SUCCESS://添加成功
                        Toast.makeText(AddFriendAct.this, "添加成功", Toast.LENGTH_SHORT).show();
                        break;
                    case XmppConstant.XMPP_ADD_FRIEND_FAIL://添加失败
                        Toast.makeText(AddFriendAct.this, "添加失败", Toast.LENGTH_SHORT).show();
                        break;
                    case XmppConstant.XMPP_ADD_FRIEND_RESIVE://有该好友了
                        Toast.makeText(AddFriendAct.this, "你们已经是好友了，不需要重新添加。", Toast.LENGTH_SHORT).show();
                        break;
                }
            }
        }.execute();
